(function initVigenereCipher(global) {
  ...
  /**
   * Clear the chi-squared memoization cache used for column-shift scoring.
   *
   * Resets the internal `chiMemoCache` to an empty Map, invalidating any previously cached chi results.
   */
  ...
  /**
   * Return the current time in milliseconds, preferring a high-resolution timer when available.
   * @returns {number} Current timestamp in milliseconds; uses `performance.now()` for higher-resolution timing when available, otherwise `Date.now()`.
   */
  function nowMs() {
  ...
  /**
   * Create an empty statistics object for collecting counters and timing during optimization and cracking.
   * @returns {{chiCalls:number, chiMemoHits:number, chiMemoMisses:number, rankedShiftCalls:number, localSearchTrials:number, localSearchEarlyStops:number, localSearchRounds:number, localSearchImprovements:number, localSearchPredictedSkips:number, statesExpanded:number, statesPruned:number, scoreEvaluations:number, totalMs:number}} An object initialized to zero with the following numeric properties:
   * - `chiCalls`: count of chi-squared computations performed.
   * - `chiMemoHits`: number of chi results retrieved from memoization cache.
   * - `chiMemoMisses`: number of chi computations not found in cache.
   * - `rankedShiftCalls`: count of times shifts were ranked for a column.
   * - `localSearchTrials`: number of local-search attempts performed.
   * - `localSearchEarlyStops`: number of local-search runs stopped early due to heuristics.
   * - `localSearchRounds`: total rounds executed across local-search runs.
   * - `localSearchImprovements`: count of observed improvements from local-search.
   * - `localSearchPredictedSkips`: estimated skips predicted during local-search optimization.
   * - `statesExpanded`: number of search states expanded.
   * - `statesPruned`: number of search states pruned.
   * - `scoreEvaluations`: number of candidate scoring evaluations performed.
   * - `totalMs`: accumulated elapsed time in milliseconds.
   */
  ...
  /**
   * Build an optimization context from provided options by normalizing optimization flags, clamping derived values, and preparing optional statistics.
   *
   * @param {Object} [options] - Configuration overrides.
   * @param {boolean|Object} [options.optimizations] - If `true`, enable default optimizations; if an object, merge its properties with default optimization flags; otherwise use no-op flags.
   * @param {boolean} [options.collectStats] - When `true`, ensure statistics collection is enabled on the resulting context.
   * @returns {{flags: Object, enabled: boolean, stats: Object|null}} An object containing:
   *  - `flags`: the normalized optimization flags (with `localSearchK` clamped to the range 1–26),
   *  - `enabled`: `true` when any substantive optimization is active,
   *  - `stats`: a newly created stats object when `flags.collectStats` is `true`, otherwise `null`.
   */
  ...
  /**
   * Normalize and validate a raw key string for Vigenère operations.
   *
   * Expands German umlauts (Ä/Ö/Ü/ä/ö/ü → Ae/Oe/Ue/ae/oe/ue), replaces ß with "ss", removes all non-letter characters, and converts the result to uppercase.
   * @param {string} rawKey - The input key string to normalize.
   * @returns {string} The normalized key containing only uppercase A–Z letters.
   * @throws {Error} If the normalized key contains no letters.
   */
  ...
  /**
   * Convert a raw key string into per-letter numeric shifts for Vigenère-style operations.
   * @param {string} key - Raw key; will be normalized (letters-only, German umlaut expansion, uppercased) before conversion.
   * @returns {number[]} Array of shifts where `A` → 0, `B` → 1, …, `Z` → 25.
   */
  ...
  /**
   * Convert an array of numeric shifts into an uppercase key string.
   * @param {number[]} shifts - Array of shifts where 0 maps to 'A' and 25 maps to 'Z'.
   * @returns {string} Uppercase key formed by mapping each shift to its corresponding letter (A–Z).
   */
  ...
  /**
   * Shift a single ASCII letter by a given amount while preserving its case.
   * @param {string} char - A single character to shift; expected to be A–Z or a–z.
   * @param {number} shift - Integer shift amount where positive moves forward in the alphabet and negative moves backward; wraps around.
   * @returns {string|null} The resulting shifted character with the same case as the input, or `null` if `char` is not an ASCII letter.
   */
  ...
  /**
   * Apply a repeating sequence of alphabetic shifts to a string, preserving non-letter characters.
   * @param {string} text - Input text to transform; non-letter characters are left unchanged.
   * @param {number[]} shifts - Array of numeric shifts (0–25) applied cyclically per letter.
   * @param {number} direction - Multiplier applied to each shift (use `1` to apply shifts forward, `-1` to apply shifts backward).
   * @returns {string} The transformed string with the shifts applied to alphabetic characters; key position advances only when a letter is processed.
   */
  ...
  /**
   * Apply a Vigenère-style cipher to the given text using the provided key and direction.
   *
   * @param {string} text - Input text; non-letter characters are preserved unchanged.
   * @param {string} key - Key string of letters used to derive per-letter shifts.
   * @param {number} direction - Use `1` to encrypt (shift forward) or `-1` to decrypt (shift backward).
   * @returns {string} The resulting text after applying the Vigenère shifts (encryption for `1`, decryption for `-1`).
   */
  ...
  /**
   * Extracts uppercase ASCII letters A–Z from the input string, preserving their original order.
   * @param {string} text - Input string to process.
   * @returns {string[]} An array of single-character uppercase letters (`A`–`Z`) appearing in the input, in order.
   */
  ...
  /**
   * Distributes a sequence of letters into a fixed number of cyclic columns.
   * @param {string} letters - Sequence of letters to distribute (order preserved).
   * @param {number} keyLength - Number of columns to create.
   * @returns {Array<Array<string>>} An array of length `keyLength` where each entry is an array of single-character strings taken from `letters` that belong to that column, in original order.
   */
  ...
  /**
   * Compute the chi-squared goodness-of-fit of a column of ciphertext letters under a candidate shift.
   *
   * @param {string[]} columnLetters - Array of uppercase letters (A–Z) from a single column; order is irrelevant.
   * @param {number} shift - Shift to apply (0–25) where 0 means no shift.
   * @param {object} [optimizationContext] - Optional optimization context; if provided, its `flags.memoChi` enables memoization and `stats` (if present) will be updated with call and cache counters.
   * @param {string} [fingerprintOverride] - Optional string used as the cache fingerprint instead of joining `columnLetters`.
   * @returns {number} The chi-squared score (lower indicates closer match to expected letter frequencies); returns `Infinity` when `columnLetters` is empty.
   */
  ...
  /**
   * Rank all 26 letter shifts for a column of letters by their chi-squared score.
   * @param {string[]} columnLetters - Array of letters (A–Z) from a single Vigenère column to evaluate.
   * @param {object} [optimizationContext] - Optional optimization context; if present and contains a `stats` object, that object's `rankedShiftCalls` counter will be incremented.
   * @returns {{shift: number, chi: number}[]} An array of 26 objects sorted by increasing `chi`, where `shift` is an integer 0–25 and `chi` is the chi-squared score for applying that shift to the column.
   */
  ...
  /**
   * Scores how well a letters-only string can be covered by entries from the compact lexicon.
   *
   * Uses a dynamic-programming based segmentation to maximize the total number of matched letters
   * (up to parts of length 12) minus a fixed penalty for internal splits, and normalizes the
   * result to the range [0, 1]. Very short inputs (fewer than 8 letters) or strings with no
   * valid segmentation return 0.
   *
   * @param {string} lettersOnly - String containing only uppercase A–Z letters to evaluate.
   * @returns {number} A normalized coverage score between 0 and 1 (`0` means no meaningful coverage, `1` means full coverage).
   */
  ...
  /**
   * Estimate a numeric confidence boost for a plaintext candidate using compact-lexicon coverage and short-word matches.
   *
   * Computes a score that favors candidates containing known short words or high compact-lexicon coverage; empty or
   * non-alphabetic inputs produce 0. Whole-word matches from the compact lexicon yield a large, length-aware boost.
   *
   * @param {string} text - Candidate plaintext to evaluate.
   * @returns {number} A non-negative boost value (higher indicates greater likelihood of meaningful text); 0 for empty or non-alphabetic inputs.
  ...
  /**
   * Clamp a numeric value to the range 0 through 1, treating non-finite inputs as 0.
   * @param {number} value - The numeric value to clamp.
   * @returns {number} The input clamped to [0, 1]; returns 0 for non-finite inputs.
   */
  ...
  /**
   * Compute a set of "sense" metrics for a piece of text measuring lexicon coverage, token meaningfulness, bigram gibberish, and an aggregated sense score.
   * @param {string} text - Input text to evaluate.
   * @returns {{dictCoverageProxy: number, meaningfulTokenRatio: number, nonsenseRatio: number, gibberishBigramRatio: number, senseScore: number}} An object of ratios in the range [0,1]:
   *  - dictCoverageProxy: proxy for dictionary/lexicon coverage (higher means more dictionary-like).
   *  - meaningfulTokenRatio: fraction of tokens judged meaningful by lexicon/segmentation heuristics.
   *  - nonsenseRatio: `1 - meaningfulTokenRatio`, representing token-level nonsense fraction.
   *  - gibberishBigramRatio: fraction of adjacent-letter bigrams that are uncommon (higher means more gibberish).
   *  - senseScore: aggregated score combining coverage, meaningful tokens, and inverse gibberish (higher means more likely to be sensible language).
   */
  ...
  /**
   * Compute a combined confidence score and sense metrics for a plaintext candidate used to prioritize brute-force fallback results.
   * @param {string} text - Plaintext candidate to evaluate.
   * @returns {{sense: {dictCoverageProxy: number, meaningfulTokenRatio: number, nonsenseRatio: number, gibberishBigramRatio: number, senseScore: number}, confidence: number}} An object containing:
   *  - `sense`: detailed sense metrics about the candidate text:
   *      - `dictCoverageProxy`: proxy for dictionary coverage,
   *      - `meaningfulTokenRatio`: fraction of tokens that appear meaningful,
   *      - `nonsenseRatio`: fraction of tokens classified as nonsense,
   *      - `gibberishBigramRatio`: proportion of unlikely bigrams,
   *      - `senseScore`: aggregated sense value.
   *  - `confidence`: numeric score combining language plausibility, dictionary-based boosts, and weighted sense heuristics to rank candidates.
   */
  ...
  /**
   * Normalize and clamp bruteforce fallback configuration from an options object.
   *
   * Parses options.bruteforceFallback, applies sensible defaults and bounds to each field,
   * validates and sanitizes stage widths (ensuring a final full-width stage of 26),
   * and returns a normalized configuration object suitable for the cracking engine.
   *
   * @param {object} options - Root options object that may contain a `bruteforceFallback` object.
   * @returns {{enabled: boolean, maxKeyLength: number, shortTextMaxLetters: number, maxTotalMs: number, maxMsPerLength: number, stageWidths: number[]}}
   * An object with the following properties:
   * - enabled: `true` if the fallback is enabled, `false` if explicitly disabled.
   * - maxKeyLength: maximum key length to brute-force (clamped between 1 and 6).
   * - shortTextMaxLetters: maximum letter count considered "short" (clamped between 4 and 256).
   * - maxTotalMs: maximum total milliseconds allowed for brute-force work (clamped between 50 and 600000).
   * - maxMsPerLength: maximum milliseconds allowed per tested key length (clamped between 50 and `maxTotalMs`).
   * - stageWidths: sorted array of valid stage widths (each 1–26) with duplicates removed and guaranteed to include 26.
   */
  ...
  /**
   * Decides whether to trigger the brute-force fallback for short ciphertexts.
   * @param {Object} context - Evaluation context used to decide the gate.
   * @param {Object} context.config - Brute-force fallback configuration.
   * @param {boolean} context.config.enabled - Global enable flag for the fallback.
   * @param {number} context.config.maxKeyLength - Maximum key length allowed for fallback.
   * @param {number} context.config.shortTextMaxLetters - Maximum letter count considered "short" for fallback.
   * @param {number} context.keyLength - Key length under consideration.
   * @param {number} context.lettersCount - Count of alphabetic characters in the ciphertext.
   * @param {Object} context.sense - Precomputed sense metrics for the best non-bruteforce candidate.
   * @param {number} context.sense.dictCoverageProxy - Compact-lexicon coverage proxy (0..1).
   * @param {number} context.sense.senseScore - Combined sense score (higher means more plausible plaintext).
   * @param {number} context.sense.nonsenseRatio - Estimated ratio of nonsense tokens (0..1).
   * @returns {{triggered: boolean, reason: string}} Object indicating whether the fallback should run and a short reason code.
   *   The `reason` will be one of:
   *     - "disabled"
  ...
  /**
   * Maintain a top-N list of candidates by `confidence`, inserting the new candidate or replacing the lowest-confidence entry.
   * Mutates the `candidates` array in place to ensure it contains at most `maxCount` items with the highest `confidence` values.
   * @param {Array<Object>} candidates - Mutable array of candidate objects, each expected to have a numeric `confidence` property.
   * @param {Object} candidate - Candidate object to consider; must have a numeric `confidence` property.
   * @param {number} maxCount - Maximum number of candidates to retain in `candidates`.
   */
  ...
  /**
   * Performs a time-limited, staged brute-force search over shift combinations for a given key length and returns scored candidates.
   *
   * @param {string} text - The ciphertext to test; non-letter characters are preserved in candidate plaintexts.
   * @param {number} keyLength - The target key length (number of shift columns) to brute-force.
   * @param {Object} config - Brute-force configuration; must include `stageWidths` (array of integers) which control how many top shifts per column are explored at each stage.
   * @param {Object} optimizationContext - Optimization context used for per-column ranking and scoring heuristics.
   * @param {number} maxElapsedMs - Maximum elapsed time in milliseconds to spend searching; the function will stop early if this budget is exceeded.
   * @returns {{best: (Object|null), candidates: Object[], combosVisited: number, elapsedMs: number}} An object with:
   *   - `best`: the highest-confidence candidate found or `null` if none,
   *   - `candidates`: a list of top unique candidates (pruned and sorted by confidence),
   *   - `combosVisited`: the number of shift combinations evaluated,
   *   - `elapsedMs`: actual elapsed time in milliseconds for the search.
   */
  ...
  /**
   * Decide whether a brute-force fallback candidate should replace the base scored candidate.
   *
   * Compares confidence and sense metrics and accepts the fallback only if it exceeds the base by
   * the configured thresholds: confidence gain >= 1.15, senseScore gain >= 0.04, and not lower
   * meaningfulTokenRatio.
   * @param {Object} baseScored - The current best candidate with at least `confidence` and `sense` properties.
   * @param {Object} fallbackCandidate - The brute-force fallback candidate with `confidence` and `sense` properties.
   * @returns {boolean} `true` if the fallbackCandidate's `confidence` is at least 1.15 times the base's confidence,
   * and its `sense.senseScore` is greater by at least 0.04, and its `sense.meaningfulTokenRatio` is
   * greater than or equal to the base's; `false` otherwise.
   */
  ...
  /**
   * Compute a heuristic score indicating how likely a string is natural language text.
   *
   * The score combines signals from common word matches, common bigrams/trigrams,
   * letter-frequency chi-squared distance, vowel ratio, compact-lexicon coverage,
   * spacing characteristics, and penalties for long repeated characters. Higher
   * values indicate greater plausibility as natural language; lower values
   * indicate gibberish or non-language-like text.
   *
   * @param {string} text - The text to evaluate.
   * @returns {number} A numeric plausibility score where larger numbers mean the text is more language-like.
  ...
  /**
   * Compute the index of coincidence for a sequence of uppercase A–Z letters.
   *
   * @param {string} columnLetters - Sequence of letters A–Z (uppercase) representing a single column.
   * @returns {number} The index of coincidence (a probability in [0,1]); returns 0 if fewer than two letters are provided.
   */
  ...
  /**
   * Compute the average index of coincidence across columns for a candidate key length.
   * @param {string} text - Input text; only letters A–Z are considered when computing columns.
   * @param {number} keyLength - Candidate key length (number of cyclic columns).
   * @returns {number} The mean IoC across all columns; `0` if the text contains no letters.
   */
  ...
  /**
   * Selects plausible Vigenère key lengths to test for the given ciphertext.
   *
   * Evaluates average index-of-coincidence across candidate lengths (up to a heuristic maximum based on letter count
   * and optimizationContext) and returns the most promising lengths. If no alphabetic letters are present, returns [1].
   * If a numeric keyLengthHint is provided, returns that hint clamped to [1, number of letters].
   *
   * @param {string} text - Ciphertext to analyze.
   * @param {number|undefined|null} keyLengthHint - Optional numeric hint for the key length; when present the hint is used directly.
   * @param {object|undefined|null} optimizationContext - Optional context whose `enabled` flag widens the candidate range when true.
   * @returns {number[]} Array of candidate key lengths (positive integers), ordered by priority and always including 1.
   */
  ...
  /**
   * Number of top shift candidates to consider for each column based on the key length and whether the length was hinted.
   * @param {number} keyLength - The candidate key length.
   * @param {boolean} hinted - True if the key length was provided as a hint.
   * @returns {number} The count of top shifts to consider per column.
   */
  ...
  /**
   * Clamp a numeric value to an integer within a specified inclusive range.
   * @param {number} value - The value to clamp; will be rounded down to an integer.
   * @param {number} minimum - Inclusive minimum allowed integer.
   * @param {number} maximum - Inclusive maximum allowed integer.
   * @returns {number} The input value rounded down with Math.floor and constrained to the range [minimum, maximum].
   */
  ...
  /**
   * Compute the candidate search budget (number of shift-state candidates to consider) for a given key length and context.
   *
   * Accepts an optional options.candidateBudget which may be the strings `"full"`, `"n^L"`, or `"26^L"` to request exhaustive search, or a positive finite number which will be clamped to the range [32, 26^L]. When no explicit budget is provided the function returns sensible defaults: a larger default for non-hinted runs, and smaller defaults for hinted runs (with an elevated default if short-text rescue is requested).
   *
   * @param {Object|null} options - Optional settings object that may contain `candidateBudget`.
   * @param {boolean} hinted - Whether a key-length hint was supplied.
   * @param {number} keyLength - The candidate key length (positive integer).
   * @param {boolean} shortTextRescue - Whether short-text rescue mode is enabled.
   * @returns {number} The resolved candidate budget (integer number of candidate states to evaluate).
   */
  ...
  /**
   * Determine the per-length state budget (number of search states) to use for a candidate key length.
   *
   * If options.stateBudget is a finite positive number, it is used after clamping to the range [64, max(64, candidateBudget)].
   * Otherwise the function returns the smaller of candidateBudget and a default cap that depends on whether a key-length hint
   * is present and whether a short-text rescue mode is active:
   * - hinted & shortTextRescue → higher default cap
   * - hinted & not shortTextRescue → moderate default cap
   * - not hinted & shortTextRescue → intermediate default cap
   * - not hinted & not shortTextRescue → lower default cap
   *
   * @param {object|null|undefined} options - Optional configuration object which may contain a numeric `stateBudget`.
   * @param {number} candidateBudget - The computed candidate budget for this key length; used as an upper bound.
   * @param {boolean} hinted - True when a key-length hint was provided (enables larger defaults).
   * @param {boolean} shortTextRescue - True when running the short-text rescue mode (adjusts caps upward).
   * @returns {number} An integer state budget to use for expanding search states (at least 64 and not exceeding candidateBudget or the configured cap).
   */
  ...
  /**
   * Compute a bounded evaluation-budget (number of candidate evaluations) for a given state budget.
   *
   * The returned integer respects a minimum of 32 and will never exceed `stateBudget`. If `options.evaluationBudget`
   * is provided and is a positive finite number it will be clamped into the valid range. Otherwise a heuristic default
   * cap is used that depends on whether a length hint was supplied (`hinted`) and whether the short-text rescue path
   * is enabled (`shortTextRescue`).
   *
   * @param {object|null} options - Optional configuration object that may contain `evaluationBudget`.
   * @param {number} stateBudget - The available per-length state budget; the evaluation budget will never exceed this.
   * @param {boolean} hinted - True when a key-length hint was provided (reduces default caps).
   * @param {boolean} shortTextRescue - True when short-text rescue mode is active (increases default caps but still bounded).
   * @returns {number} An integer evaluation budget between 32 and `stateBudget`, chosen from `options.evaluationBudget` when valid or from heuristic defaults otherwise.
   */
  ...
  /**
   * Selects a diverse subset of ranked shift options prioritizing low-chi entries while optionally adding deep diversity from the tail.
   *
   * @param {{chi: number}[]} ranks - Array of shift option objects ordered by increasing chi (best first).
   * @param {number} optionCount - Number of options to select (clamped to ranks.length).
   * @param {boolean} includeDeepDiversity - If true, include some widely spaced options from the tail to increase diversity.
   * @return {{chi: number}[]} An array of the selected shift option objects, sorted by increasing `chi`.
   */
  ...
  /**
   * Insert an item into a max-heap (array) ordered by the `chi` property.
   * @param {Array<Object>} heap - Array representing a binary max-heap; items must have a numeric `chi` property.
   * @param {Object} item - Item to insert; must include a numeric `chi` value used for ordering.
   */
  ...
  /**
   * Replace the root of a max-heap (ordered by the `chi` property) with `item` and restore the heap property in place.
   * @param {Array<{chi:number}>} heap - Array representing a max-heap where each element has a numeric `chi` property and heap[0] is the current root. This array is mutated.
   * @param {{chi:number}} item - Element to place at the root; must have a numeric `chi` property used for ordering.
   */
  ...
  /**
   * Expand a set of partial key states by combining each with candidate column options while enforcing a budget.
   *
   * Generates next-layer state combinations (each with `shifts`, `chi`, and `partialChi`) by concatenating an option's shift onto each input state, using either full expansion or budgeted exploration with progressive widening and chi-based pruning driven by the optimizationContext flags and stateBudget.
   *
   * @param {Array<{shifts: number[], chi: number, partialChi: number}>} states - Current partial states to expand.
   * @param {Array<{shift: number, chi: number}>} options - Candidate shifts for the next column, each with its chi score.
   * @param {number} stateBudget - Maximum number of states to keep when budgeted (full expansion if states.length * options.length <= stateBudget).
   * @param {{flags: Object, stats?: Object}|null} optimizationContext - Context controlling optimizations and optional stats counters; if null, no optimizations or stats are applied.
   * @param {number} layerIndex - Zero-based index of the column being expanded (used to decide partialChi accounting).
   * @param {number} subsetColumnCount - Number of initial columns included in partialChi aggregation.
   * @returns {Array<{shifts: number[], chi: number, partialChi: number}>} Sorted array of expanded states (ascending by `chi`) truncated according to the budget and pruning heuristics.
   */
  ...
  /**
   * Construct candidate shift states by expanding per-column shift options while respecting a state budget.
   *
   * @param {Array<Array<Object>>} columnOptions - An array where each element is a list of ranked shift option objects for a column (ordered by increasing chi).
   * @param {number} stateBudget - Maximum number of combined states to retain during expansion for each step.
   * @param {Object} optimizationContext - Optimization settings and optional stats that influence expansion/pruning behaviour.
   * @returns {Array<{shifts: number[], chi: number, partialChi: number}>} An array of candidate state objects. Each state contains `shifts` (array of per-column shift indices), `chi` (accumulated chi score), and `partialChi` (auxiliary partial chi used during incremental expansion).
   */
  ...
  /**
   * Produce an ordered representative sample of up to `sampleCount` items from `items`, favoring earlier elements.
   * @param {Array} items - Source array to sample from.
   * @param {number} sampleCount - Desired number of samples; clamped to the input length.
   * @returns {Array} An array of sampled elements (in the same relative order as `items`), with length equal to the smaller of `sampleCount` and `items.length`. */
  ...
  /**
   * Decode a column of uppercase A–Z letters by applying a single Caesar shift.
   * @param {string} columnLetters - Sequence of uppercase letters A–Z representing a column from the ciphertext.
   * @param {number} shift - Shift amount in range 0–25 to subtract from each letter (A=0).
   * @returns {string} Decoded uppercase letters after applying the shift.
   */
  ...
  /**
   * Estimates the change in a candidate's score if the shift at a given column is replaced.
   *
   * Uses a lightweight heuristic combining the change in chi-squared for the column and a small
   * vowel-ratio correction to predict whether switching the column's shift to `newShift` will
   * improve the overall score.
   *
   * @param {Object} state - Current search state. Must include `shifts` (array of current shifts),
   *   `columns` (array of letter strings per column), `columnFingerprints` (fingerprint per column),
   *   `optimizationContext` (options for chi calculation), and `decodedColumnCache` (array of Maps for memoized decoded columns).
   * @param {number} pos - Index of the column/shift to change.
   * @param {number} newShift - Candidate shift (0–25) to evaluate for the column.
   * @returns {number} A numeric delta estimate where a positive value indicates the change is
   *   expected to improve the candidate's score and a negative value indicates an expected decline.
  ...
  /**
   * Performs a focused local search over the per-column shifts to improve the decrypted text's language score.
   *
   * Uses the provided initial shifts as a starting point and attempts to find shift changes that increase the language score.
   * If an optimizationContext is supplied its flags (for example `localSearchK` and `incrementalScoring`) and stats object will be consulted/updated during the search.
   *
   * @param {string} text - The original ciphertext (may include non-letter characters which are preserved).
   * @param {number[]} initialShifts - Array of integers 0–25 representing the starting per-column shifts (A=0).
   * @param {{flags?: Object, stats?: Object, enabled?: boolean}|null} optimizationContext - Optional optimization context containing runtime flags and an optional stats accumulator; may be null to disable optimizations and stats.
   * @returns {{key: string, text: string, score: number}} An object with the best key (string), the best decoded text found, and its language score.
  ...
  /**
   * Filters and returns up to maxCount unique candidates by their key and text identity.
   * @param {Array<{key: string, text: string}>} candidates - Ordered list of candidate objects to deduplicate.
   * @param {number} maxCount - Maximum number of unique candidates to return.
   * @returns {Array<{key: string, text: string}>} An array of unique candidates (preserving input order), length <= maxCount.
   */
  ...
  /**
   * Attempts to recover a Vigenère key of a specific length from ciphertext and returns scored candidate plaintexts.
   *
   * @param {string} text - The ciphertext to analyze; non-letter characters are preserved in candidates.
   * @param {number} keyLength - The key length to assume for this cracking run (values < 1 are treated as 1).
   * @param {boolean} hinted - Whether the caller supplied this length as a hint (enables heuristic adjustments).
   * @param {object} [options] - Optional tuning and optimization flags that influence budgeting, memoization, and search behavior.
   * @returns {object} An object with three properties:
   *  - best: { key: string, text: string, confidence: number } — the top candidate key, its decoded text, and confidence score.
   *  - candidates: Array<{ key: string, text: string, confidence: number, source?: string }> — ranked candidate list (deduplicated).
   *  - search: { shortTextRescue: boolean, optionCount: number, candidateBudget: number, stateBudget: number, evaluationBudget: number, statesGenerated: number, statesEvaluated: number, optimizations: object, telemetry?: object, exhaustive?: boolean, combos?: number, dictionaryBoostApplied?: boolean } — telemetry and budget/strategy metadata about this run.
   */
