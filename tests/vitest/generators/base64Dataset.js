/**
 * Deterministischer Base64-Datensatz fuer Qualitaetsgates.
 *
 * Ziel: feste Pflichtfaelle plus reproduzierbare Zufallsfaelle,
 * damit Base64-Decode/Crack Aenderungen stabil bewertet werden koennen.
 */
...
function encodeBase64Text(text) {
  // Eigene Base64-Implementierung haelt den Datensatz Buffer-frei und konsistent zum Cipher.
  return encodeBase64(encodeUtf8(text));
}
...
function generateBase64Dataset(n, seed = 0) {
  ...
  for (const mandatoryCase of MANDATORY_CASES) {
    ...
    dataset.push({
      ...
      ciphertext: encodeBase64Text(mandatoryCase.plaintext),
      mandatory: true,
    });
  }
  ...
  while (dataset.length < n) {
    ...
    if (attempts > maxAttempts) {
      throw new Error(
        `generateBase64Dataset: reached maxAttempts (${maxAttempts}) while building dataset of size ${n}. ` +
    ...
    dataset.push({
      id: dataset.length,
      plaintext,
      ciphertext: encodeBase64Text(plaintext),
      mandatory: false,
    });
  }
