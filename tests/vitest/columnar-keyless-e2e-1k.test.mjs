...
const { generateColumnarDataset } = require("./generators/columnarDataset.js");
...
describe("columnar keyless e2e 1k", () => {
  it("builds a deterministic 1000-case dataset", () => {
    const datasetA = generateColumnarDataset(1000, 42);
    const datasetB = generateColumnarDataset(1000, 42);
    ...
    expect(signatures.has("3-1-4-2|POTENTIALTOPF MODELL")).toBe(true);
  ...
  it(
    ...
    () => {
      ...
      const dataset = generateColumnarDataset(1000, 42);
      ...
      for (const testCase of dataset) {
        const unhinted = columnar.crack(testCase.ciphertext);
        const hinted = columnar.crack(testCase.ciphertext, { keyLength: testCase.keyLength });
