...
const { generateBinaryDataset } = require("./generators/binaryDataset.js");
...
describe("binary (8-bit) keyless e2e 1k", () => {
  it("builds a deterministic 1000-case dataset", () => {
    const datasetA = generateBinaryDataset(1000, 42);
    const datasetB = generateBinaryDataset(1000, 42);
    ...
    expect(signatures.has("UNSCHAERFE IM ORT")).toBe(true);
  ...
  it(
    ...
    () => {
      ...
      const dataset = generateBinaryDataset(1000, 42);
      ...
      for (const testCase of dataset) {
        const decrypted = binaryCipher.decrypt(testCase.ciphertext);
        const cracked = binaryCipher.crack(testCase.ciphertext);
      ...
      // Gate schuetzt Binär-Decode vor regressiven Sonderfaellen.
